...
const conses2array = (process.env.NODE_ENV === 'production'
  ? id
  : C.res(I.freeze))(function uncons(cs) {
  const xs = []
  while (null !== cs) {
    if (!isPair(cs)) return undefined
    xs.push(cs[0])
    cs = cs[1]
  }
  return xs.reverse()
...
const array2conses = (process.env.NODE_ENV === 'production'
  ? id
  : fn => x => {
      const r = fn(x)
      for (let x = r; null != x; x = x[1]) I.freeze(x)
      return r
    })(xs => {
  if (I.isArray(xs)) {
    let cs = null
    for (let i = 0, n = xs[I.LENGTH]; i < n; ++i) cs = [xs[i], cs]
    return cs
  }
})
...
const unfoldPartial = (process.env.NODE_ENV === 'production'
  ...
  sIas =>
    function unfold(s) {
      const xs = []
      for (;;) {
        const as = getU(sIas, s)
        if (!I.isArray(as) || as[I.LENGTH] !== 2) return [xs, s]
        s = as[1]
        xs.push(as[0])
      }
    }
...
const foldPartial = sIas => xss => {
  if (isPair(xss)) {
    const xs = xss[0]
    if (I.isArray(xs)) {
      let s = xss[1]
      let n = xs[I.LENGTH]
      while (n--) s = setU(sIas, [xs[n], s], void 0)
      return s
    }
  }
}
...
export const unfold = sIas => isoU(unfoldPartial(sIas), foldPartial(sIas))
...
export const uncons = isoU(conses2array, array2conses)
